 ...
 * 从报告文本中提取关键段落，写回动态文件
 * 注意：必须严格匹配字段语义——
 *   - `batna` ← "BATNA / WATNA 对比"  (语义匹配：双方最佳替代方案)
 *   - 不再写入 `positions` / `potentialInterests`：
 *     原代码将"案件关键信息摘要"写入 `positions`、将"推荐方案与理由"
 *     写入 `potentialInterests`，会污染后续 Agent 上下文。
 *     报告原文已通过 API 响应 `data.content` 返回给调用方，无需再回写。
 */
