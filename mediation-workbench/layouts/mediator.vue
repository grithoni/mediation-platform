...
// 调解员工作台布局（左侧边栏）
// 侧边栏收起状态：useState 保持，切换页面不丢失
const sidebarCollapsed = useState<boolean>('mediatorSidebarCollapsed', () => false)
...
const navItems = [
  ...
  { key: 'desensitize', icon: 'i-lucide-shield-check', label: '脱敏规则', to: '/mediator/desensitize' },
...
<template>
  <div class="min-h-screen flex bg-gray-50 dark:bg-gray-950">
    <!-- ── 左侧边栏 ── -->
    <aside
      class="shrink-0 overflow-hidden transition-all duration-300 ease-in-out bg-white dark:bg-gray-900"
      :class="sidebarCollapsed ? 'w-0 border-r-0' : 'w-60 border-r border-gray-200 dark:border-gray-800'"
    >
    ...
    <main class="flex-1 min-w-0 flex flex-col">
      ...
      <!-- 侧边栏收起时的再展开入口（悬浮于内容区左缘中部） -->
      <button
        v-if="sidebarCollapsed"
        class="fixed left-3 top-1/2 -translate-y-1/2 z-30 w-7 h-7 rounded-full flex items-center justify-center border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 text-gray-500 dark:text-gray-400 shadow-md hover:text-blue-600 dark:hover:text-blue-400 hover:border-blue-300 dark:hover:border-blue-700 transition-colors"
        aria-label="展开侧边栏"
        title="展开侧边栏"
        @click="sidebarCollapsed = false"
      >
        <UIcon name="i-lucide-panel-left-open" class="w-4 h-4" />
      </button>
      <slot />
    </main>
  </div>
