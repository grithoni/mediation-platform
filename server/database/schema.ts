...
// ============================================================
// 调解员表 (保留兼容，关联 users 表)
// ============================================================
export const mediators = sqliteTable('mediators', {
  id: text('id').primaryKey(),
  userId: text('user_id').references(() => users.id), // 关联用户表
  tenantId: text('tenant_id').references(() => tenants.id),
  name: text('name').notNull(),
  username: text('username').notNull().unique(),
  email: text('email'),
  ...
  hasForeignCapability: integer('has_foreign_capability', { mode: 'boolean' }),
  foreignLanguages: text('foreign_languages'),
  foreignLanguageLevel: text('foreign_language_level'),
  appointmentType: text('appointment_type'),
  organization: text('organization'),
  position: text('position'),
  categoryTypes: text('category_types'), // JSON array
  learningAndWorkExperience: text('learning_and_work_experience'),
  // 统计字段
  totalCases: integer('total_cases').default(0),
  successCases: integer('success_cases').default(0),
